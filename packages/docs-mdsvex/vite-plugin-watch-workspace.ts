...
const getOutDir = (file: string, tsconfig: { [key: string]: any }) => {
    ...
    const outFolder = tsconfig.compilerOptions.outDir ? tsconfig.compilerOptions.outDir.replace(RELATIVE_PATH_REGEX, '') : '';
 ...
 */
console.log('load');
export async function VitePluginWatchWorkspace(config: VitePluginWatchExternalOptions): Promise<Plugin<any>> {
