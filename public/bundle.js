var SvelteGantt = (function () {
	...
	function data() {
	    return {
	        dragging: false,
	        gantt: null,
	        onsuccess: function(targetRow, dropDate, ganttUtils) {
	            const newTask = new SvelteTask({
	...
	function drag(node) {
	    ...
	    function onmouseup(event) {
	        ...
	        if(successful){
	            ...
	            const dropDate = ganttUtils.getDateByPosition(mousePos.x);
	            ...
	            if(rowElement !== undefined) {
	                ...
	                onsuccess(targetRow, dropDate, ganttUtils);
	...
	function create_main_fragment$2(component, ctx) {
		var div, div_1, text, text_1_value = ctx.task.label, text_1, drag_action, contextMenu_action, selectable_action, current;
		...
		return {
			c: function create() {
				...
				div_1.className = "task-background svelte-13h7vne";
				setStyle(div_1, "width", "" + ctx.task.amountDone + "%");
				addLoc(div_1, file$2, 9, 4, 283);
				div.className = "task svelte-13h7vne";
	...
	function create_main_fragment$6(component, ctx) {
		...
		return {
			c: function create() {
				...
				div.className = "column-header-row svelte-vtosw2";
				setStyle(div, "width", "" + ctx.width + "px");
	...
	function create_each_block$2(component, ctx) {
		...
		return {
			c: function create() {
				div = createElement("div");
				text = createText(text_value);
				div.className = "column-header svelte-vtosw2";
				setStyle(div, "width", "" + ctx.header.width + "px");
				addLoc(div, file$6, 3, 8, 98);
			},
	...
	assign(Arrow.prototype, methods$4);
	...
	assign(Dependency.prototype, methods$5);
	    ...
	    SvelteGantt.create = function(target, data, options) {
	        ...
	        for(let i=0; i < data.rows.length; i++){
	            ...
	            for(let j=0; j < row.tasks.length; j++){
	                ...
	                allTasks.push(row.tasks[j]);
	            }
	        }

	        for(let i=0; i < data.dependencies.length; i++){
	            let dependency = data.dependencies[i];
	            const fromTask = allTasks.find(t => t.id == dependency.fromTask);
	            const toTask = allTasks.find(t => t.id == dependency.toTask);
	            dependency.fromTask = fromTask;
	            dependency.toTask = toTask;

	            data.dependencies[i] = new SvelteDependency(dependency, store.get());
	        }
	...
	function create_main_fragment$9(component, ctx) {
		...
		return {
			...
			m: function mount(target, anchor) {
				...
				append(externaldiv._slotted.default, text_16);
	...
	function Grid(options) {
		...
		init(this, options);
		this.refs = {};
		this._state = assign(assign(this.store._init(["rowHeight","width","headers","height","gantt"]), data$7()), options.data);
	...
	assign(Grid.prototype, methods$6);
