var app = (function () {
	...
	assign(Dependency.prototype, methods$2);
	...
	function oncreate$5(){
	    ...
	    for(let i=0; i < rows.length; i++){
	        Task$1.updatePosition(rows[i].tasks[0], this.store.get());
	    }
	    ...
	    SvelteGantt.create = function(target, data, options) {
	        ...
	        for(let i=0; i < data.rows.length; i++){
	            ...
	            for(let j=0; j < row.tasks.length; j++){
	                ...
	                Task$1.updatePosition(task, store.get());
	                task.row = row;
	            }
	        }
	...
	function create_each_block_4(component, ctx) {
		...
		return {
			...
			p: function update(changed, ctx) {
				...
				if (changed.dependencies) dependency_changes.fromTask = ctx.dependency.fromTask;
				if (changed.dependencies) dependency_changes.toTask = ctx.dependency.toTask;
	...
	function Grid(options) {
		...
		init(this, options);
		this.refs = {};
		this._state = assign(assign(this.store._init(["rowHeight","headerHeight","width","headers","height"]), data$3()), options.data);
	...
	assign(Grid.prototype, methods$3);
	...
	data$4.dependencies.push({fromTask: data$4.rows[0].tasks[0], toTask: data$4.rows[1].tasks[0] });
	data$4.dependencies.push({fromTask: data$4.rows[2].tasks[0], toTask: data$4.rows[1].tasks[0] });
	...
	var app = Grid.create(document.body, data$4, options);
