var app = (function () {
	...
	function create_main_fragment(component, ctx) {
		var div, text_value = ctx.task.label, text, drag_action, current;

		return {
			c: function create() {
				div = createElement("div");
				text = createText(text_value);
				div.className = "task svelte-1hp799i";
	...
	assign(Task$1.prototype, methods);
	...
	function oncreate$1() {
	    ...
	    this.on('update', ({ changed, current, previous }) => {
	        this.get().row.rowElement = this.refs.row;
	    });
	}
	...
	// (2:4) {#each tasks as task (task.id)}
	function create_each_block(component, key_1, ctx) {
		...
		return {
			...
			p: function update(changed, ctx) {
				...
				if (changed.tasks) task_changes.task = ctx.task;
	...
	function Row(options) {
		...
		if (!('tasks' in this._state)) console.warn("<Row> was created without expected data property 'tasks'");
	...
	assign(Row.prototype, methods$1);
	...
	const file$2 = "src\\RowHeader.html";
	...
	const file$5 = "src\\Arrow.html";

	function create_main_fragment$5(component, ctx) {
		var svg, path_1, path_2, path_2_d_value, current;
		...
		return {
			c: function create() {
				...
				setAttribute(path_1, "stroke", "black");
				setAttribute(path_1, "fill", "transparent");
				addLoc(path_1, file$5, 1, 2, 114);
				setAttribute(path_2, "d", path_2_d_value = "M" + (ctx.endX-5) + " " + (ctx.endY-5) + " \r\n  L" + ctx.endX + " " + ctx.endY + "\r\n  L" + (ctx.endX-5) + " " + (ctx.endY+5) + " Z");
				addLoc(path_2, file$5, 3, 2, 175);
			...
			p: function update(changed, ctx) {
				...
				if ((changed.endX || changed.endY) && path_2_d_value !== (path_2_d_value = "M" + (ctx.endX-5) + " " + (ctx.endY-5) + " \r\n  L" + ctx.endX + " " + ctx.endY + "\r\n  L" + (ctx.endX-5) + " " + (ctx.endY+5) + " Z")) {
					setAttribute(path_2, "d", path_2_d_value);
				}
			},
	...
	const file$6 = "src\\Dependency.html";
	...
	function create_main_fragment$7(component, ctx) {
		var div, div_1, text, div_2, text_2, div_3, scrollListener_action, text_4, div_4, div_5, div_6, text_6, div_7, text_8, div_8, scrollable_action, current;
		...
		for (var i = 0; i < each_value_3.length; i += 1) {
			each_3_blocks[i] = create_each_block_3(component, get_each_3_context(ctx, each_value_3, i));
		}
		...
		function outroBlock_3(i, detach, fn) {
			if (each_3_blocks[i]) {
				each_3_blocks[i].o(() => {
					if (detach) {
						each_3_blocks[i].d(detach);
						each_3_blocks[i] = null;
					}
					if (fn) fn();
				});
			}
		...
		var each_value_4 = ctx.dependencies;

		var each_4_blocks = [];

		for (var i = 0; i < each_value_4.length; i += 1) {
			each_4_blocks[i] = create_each_block_4(component, get_each_4_context(ctx, each_value_4, i));
		}
		...
		function outroBlock_4(i, detach, fn) {
			if (each_4_blocks[i]) {
				each_4_blocks[i].o(() => {
					if (detach) {
						each_4_blocks[i].d(detach);
						each_4_blocks[i] = null;
					}
					if (fn) fn();
				});
			}
		...
		return {
			c: function create() {
				...
				div_1.className = "side-header-container svelte-jqua9j";
				setStyle(div_1, "height", "" + ctx.$headerHeight + "px");
				setStyle(div_1, "width", "100px");
				addLoc(div_1, file$7, 2, 4, 26);
				div_2.className = "main-header-container svelte-jqua9j";
				setStyle(div_2, "height", "" + ctx.$headerHeight + "px");
				setStyle(div_2, "width", "" + ctx.$width + "px");
				addLoc(div_2, file$7, 6, 4, 132);
				div_3.className = "side-container svelte-jqua9j";
				setStyle(div_3, "height", "" + ctx.$height + "px");
				scrollListener_action = scrollListener.call(component, div_3) || {};
				addLoc(div_3, file$7, 12, 4, 335);
				div_6.className = "column-container svelte-jqua9j";
				addLoc(div_6, file$7, 23, 12, 688);
				div_7.className = "row-container svelte-jqua9j";
				setStyle(div_7, "padding-top", "" + ctx.paddingTop + "px");
				setStyle(div_7, "padding-bottom", "" + ctx.paddingBottom + "px");
				setStyle(div_7, "height", "1200px");
				addLoc(div_7, file$7, 28, 12, 876);
				div_8.className = "dependency-container svelte-jqua9j";
				addLoc(div_8, file$7, 38, 12, 1313);
				div_5.className = "content svelte-jqua9j";
				addLoc(div_5, file$7, 20, 8, 637);
				div_4.className = "main-container svelte-jqua9j";
				setStyle(div_4, "height", "" + ctx.$height + "px");
				setStyle(div_4, "width", "" + ctx.$width + "px");
				scrollable_action = scrollable.call(component, div_4) || {};
				addLoc(div_4, file$7, 19, 4, 540);
				div.className = "grid svelte-jqua9j";
			...
			p: function update(changed, ctx) {
				...
				if (changed.rows) {
					...
					for (; i < each_1_blocks.length; i += 1) outroBlock_1(i, 1);
				}
				...
				if (changed.columns) {
					...
					for (; i < each_2_blocks.length; i += 1) outroBlock_2(i, 1);
				}

				if (changed.visibleRows || changed.width) {
					each_value_3 = ctx.visibleRows;

					for (var i = 0; i < each_value_3.length; i += 1) {
						const child_ctx = get_each_3_context(ctx, each_value_3, i);

						if (each_3_blocks[i]) {
							each_3_blocks[i].p(changed, child_ctx);
						} else {
					...
					for (; i < each_3_blocks.length; i += 1) outroBlock_3(i, 1);
				}
				...
				if (changed.dependencies) {
					each_value_4 = ctx.dependencies;

					for (var i = 0; i < each_value_4.length; i += 1) {
						const child_ctx = get_each_4_context(ctx, each_value_4, i);

						if (each_4_blocks[i]) {
							each_4_blocks[i].p(changed, child_ctx);
						} else {
					...
					for (; i < each_4_blocks.length; i += 1) outroBlock_4(i, 1);
				}
			...
			o: function outro(outrocallback) {
				...
				for (let i = 0; i < each_1_blocks.length; i += 1) outroBlock_1(i, 0, countdown_1);

				const countdown_2 = callAfter(outrocallback, each_2_blocks.length);
				for (let i = 0; i < each_2_blocks.length; i += 1) outroBlock_2(i, 0, countdown_2);

				const countdown_3 = callAfter(outrocallback, each_3_blocks.length);
				for (let i = 0; i < each_3_blocks.length; i += 1) outroBlock_3(i, 0, countdown_3);

				const countdown_4 = callAfter(outrocallback, each_4_blocks.length);
				for (let i = 0; i < each_4_blocks.length; i += 1) outroBlock_4(i, 0, countdown_4);
	...
	function create_each_block_1(component, ctx) {
		var rowheader_updating = {}, current;
		...
		return {
			...
			p: function update(changed, _ctx) {
				...
				if (!rowheader_updating.label && changed.rows) {
	...
	// (30:16) {#each visibleRows as row}
	function create_each_block_3(component, ctx) {
		var row_updating = {}, current;

		var row_initial_data = { row: ctx.row, width: ctx.width };
		if (ctx.row.tasks !== void 0) {
			row_initial_data.tasks = ctx.row.tasks;
			row_updating.tasks = true;
		}
		var row = new Row({
			root: component.root,
			store: component.store,
			...
			_bind(changed, childState) {
				var newState = {};
				if (!row_updating.tasks && changed.tasks) {
					ctx.row.tasks = childState.tasks;
		...
		return {
			...
			p: function update(changed, _ctx) {
				...
				if (!row_updating.tasks && changed.visibleRows) {
					row_changes.tasks = ctx.row.tasks;
					row_updating.tasks = ctx.row.tasks !== void 0;
				}
	...
	function create_each_block_4(component, ctx) {
		var dependency_updating = {}, current;

		var dependency_initial_data = {};
		if (ctx.dependency.fromTask !== void 0) {
			dependency_initial_data.fromTask = ctx.dependency.fromTask;
			dependency_updating.fromTask = true;
		}
		if (ctx.dependency.toTask !== void 0) {
			dependency_initial_data.toTask = ctx.dependency.toTask;
			dependency_updating.toTask = true;
		}
		var dependency = new Dependency({
			...
			_bind(changed, childState) {
				var newState = {};
				if (!dependency_updating.fromTask && changed.fromTask) {
					ctx.dependency.fromTask = childState.fromTask;

					newState.dependencies = ctx.dependencies;
				}

				if (!dependency_updating.toTask && changed.toTask) {
					ctx.dependency.toTask = childState.toTask;
		...
		component.root._beforecreate.push(() => {
			dependency._bind({ fromTask: 1, toTask: 1 }, dependency.get());
		});
		...
		return {
			...
			p: function update(changed, _ctx) {
				...
				if (!dependency_updating.fromTask && changed.dependencies) {
					dependency_changes.fromTask = ctx.dependency.fromTask;
					dependency_updating.fromTask = ctx.dependency.fromTask !== void 0;
				}
				if (!dependency_updating.toTask && changed.dependencies) {
					dependency_changes.toTask = ctx.dependency.toTask;
					dependency_updating.toTask = ctx.dependency.toTask !== void 0;
				}
	...
	function Grid(options) {
		...
		init(this, options);
		this.refs = {};
		this._state = assign(assign(this.store._init(["headerHeight","width","headers","height"]), data$4()), options.data);
		this.store._add(this, ["headerHeight","width","headers","height"]);
		...
		if (!('width' in this._state)) console.warn("<Grid> was created without expected data property 'width'");
		if (!('dependencies' in this._state)) console.warn("<Grid> was created without expected data property 'dependencies'");
		this._intro = !!options.intro;
	...
	assign(Grid.prototype, methods$2);
	...
	data$5.dependencies.push({fromTask: data$5.rows[0].tasks[0], toTask: data$5.rows[1].tasks[0] });

	store.set({rows: data$5.rows});
	...
	const file$8 = "src\\App.html";

	function create_main_fragment$8(component, ctx) {
		var p, text, text_1, grid_updating = {}, current;

		var grid_initial_data = { dependencies: ctx.dependencies };
		if (ctx.rows !== void 0) {
			grid_initial_data.rows = ctx.rows;
			grid_updating.rows = true;
		}
		var grid = new Grid({
			root: component.root,
			store: component.store,
			data: grid_initial_data,
			_bind(changed, childState) {
				var newState = {};
				if (!grid_updating.rows && changed.rows) {
					newState.rows = childState.rows;
				}
				component._set(newState);
				grid_updating = {};
			}
		...
		component.root._beforecreate.push(() => {
			grid._bind({ rows: 1 }, grid.get());
		});

		return {
			c: function create() {
				p = createElement("p");
				text = createText("This is a top-level element.");
				text_1 = createText("\n");
				grid._fragment.c();
			...
			p: function update(changed, _ctx) {
				ctx = _ctx;
				var grid_changes = {};
				if (changed.dependencies) grid_changes.dependencies = ctx.dependencies;
				if (!grid_updating.rows && changed.rows) {
					grid_changes.rows = ctx.rows;
					grid_updating.rows = ctx.rows !== void 0;
				}
				grid._set(grid_changes);
				grid_updating = {};
			},
			...
			o: function outro(outrocallback) {
				...
				grid._fragment.o(outrocallback);
				current = false;
			},
	...
	function App(options) {
		...
		if (!('dependencies' in this._state)) console.warn("<App> was created without expected data property 'dependencies'");
		this._intro = !!options.intro;
